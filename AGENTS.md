...
4. **提交前本地必跑**：`bash scripts/ci-check.sh`，全绿才提交。
5. **修完回填 §5-§7**：键盘/布局/BGM/PWA 类必须过回归矩阵（§5），存在任何 ❌ 不许合入 main。
...
- CI 检查项见 `scripts/ci-check.sh`：内联 JS 语法 / 版本号一致 / 重复 ID 密度 / 危险 API 密度 / 提交范围。
