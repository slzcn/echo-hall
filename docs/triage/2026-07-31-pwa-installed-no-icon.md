...
## §7 补充根因（主人反馈"曾经装成功过，现在不行"后定位）

manifest.id 时间线（git 实证）：
- 07-24 15:39 前：id=`/echo-hall/`，start_url=`./` —— **主人此时装成功过**
- 07-24 22:42：id 改成 `/echo-hall/?pwa=20260724`
- 07-31 01:39 V58：id 改成 `/echo-hall/?pwa=20260731-v58`
- 07-31 10:16 V62：id 钉回 `https://slzcn.github.io/echo-hall/`

**根因**：id 是 Chrome 判定"是不是同一个 PWA"的唯一键。id 反复漂移，导致主人手机上那次成功安装留下的旧 WebAPK 记录（老 id）与新 id 对不上。安装新 id 时通知栏虽出现"正在添加"（铸造在跑），但系统里旧包记录冲突/被去重，新图标不落主屏。

**处置**：V62 已把 id 永久钉死为 start_url。但主机端旧记录需手动清：卸载所有旧「回声厅」条目 + Chrome 清该站点数据 → 用 V62 重装一次，之后 id 不再变，不会再复发。
