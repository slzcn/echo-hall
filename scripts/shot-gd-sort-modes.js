#!/usr/bin/env node
'use strict';
// 截掼蛋理牌两态(按大小 / 按牌型, 均普通叠牌排)+ 长按手动拖排。用法: node scripts/shot-gd-sort-modes.js
...
(async()=>{
  ...
  console.log('1 按大小  钮=',await btnTxt());
  await clickSort();
  await page.screenshot({path:path.join(OUT,'gdsort-2-combo.png')});
  console.log('2 按牌型  钮=',await btnTxt());
  // 按牌型态: 点中间一张 → 看单张选中(不连组)
  await page.evaluate(()=>{ const cards=[...document.querySelectorAll('.gd-hand .card')]; const t=cards[Math.floor(cards.length/2)]; const r=t.getBoundingClientRect(); const hand=document.querySelector('.gd-hand'); ['pointerdown','pointerup'].forEach(ty=>hand.dispatchEvent(new PointerEvent(ty,{bubbles:true,pointerId:1,pointerType:'touch',clientX:r.left+2,clientY:r.top+r.height/2,buttons:ty==='pointerup'?0:1}))); });
  await page.waitForTimeout(400);
  await page.screenshot({path:path.join(OUT,'gdsort-2b-combo-sel.png')});
  console.log('2b 按牌型·单张选中');
  // 横屏按牌型
  ...
  console.log('2c 按牌型·横屏');
  await ctx.pages()[0].setViewportSize({width:390,height:844});
  await page.waitForTimeout(300);
  await clickSort();
  await page.screenshot({path:path.join(OUT,'gdsort-3-back-rank.png')});
  console.log('3 切回按大小 钮=',await btnTxt());
