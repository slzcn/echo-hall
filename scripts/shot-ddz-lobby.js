#!/usr/bin/env node
'use strict';
/* 一次性: 渲染斗地主招募态(host,只我一人坐着,两空位)截图, 日间+夜间两版。
 * 用法: node scripts/shot-ddz-lobby.js <out前缀>   → /tmp/eh-lobby/<前缀>-day.png / -night.png */
const fs = require('fs');
const path = require('path');
const ROOT = path.join(__dirname, '..');
const G = f => fs.readFileSync(path.join(ROOT, 'js/games', f), 'utf8');
const PREFIX = process.argv[2] || 'head';
...
async function shot(browser, palette, tag){
  ...
  const modeAttr = tag==='day' ? ' data-mode="day"' : '';   // 真机日间态在 html 上挂 data-mode="day", 复刻它才能命中日间覆盖规则
  ...
  for (const f of ['deck.js','ddz-rules.js','ddz-engine.js','ddz-ai.js','game-ui.js']) await page.addScriptTag({ content: G(f) });
  await page.evaluate(() => {
    const noop=()=>{};
