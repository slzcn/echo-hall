#!/usr/bin/env node
'use strict';
/* 一次性: 渲染掼蛋招募态(host,只我一人坐着,三空位)截图, 日间+夜间两版。
 * 用法: node scripts/shot-guandan-lobby.js <out前缀>   → /tmp/eh-lobby/<前缀>-day.png / -night.png */
const fs = require('fs');
const path = require('path');
const ROOT = path.join(__dirname, '..');
const G = f => fs.readFileSync(path.join(ROOT, 'js/games', f), 'utf8');
const PREFIX = process.argv[2] || 'gd-head';
