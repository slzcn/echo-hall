...
elif node scripts/test-ddz-rules.js >/dev/null && node scripts/test-ddz-engine.js >/dev/null && node scripts/test-ddz-fuzz.js >/dev/null && node scripts/journey-ddz-play.js >/dev/null; then
  pass "斗地主牌型/比较 50 项 + 引擎/AI自对弈40局/回看重放 + 模糊压测400局6万项不变量(守恒/零和/重放/队友协作) + /斗地主完整旅程 全部通过"
else
