...
# 1a2. 房间生命周期竞态旅程
# ─────────────────────────────────────────
section "1a2. 房间生命周期竞态旅程"
if node scripts/journey-room-lifecycle.js; then
  pass "旧房延迟清理 / 快速进新房 / 旧实现反证通过"
else
  fail "房间生命周期竞态旅程失败"
fi
...
section "1a6. 消息发送失败恢复"
if node scripts/journey-message-send-failure.js; then
  pass "失败气泡 / 原文保留 / 原地重试 / 切房保护 / 旧实现反证通过"
else
  fail "消息发送失败恢复旅程失败"
fi

section "1a6b. 主聊天输入框焦点轮廓"
if node scripts/journey-composer-focus-outline.js; then
  pass "主聊天输入框仅保留外层焦点指示器 / 旧双边框实现反证通过"
else
  fail "主聊天输入框焦点轮廓旅程失败"
fi
...
elif node scripts/test-guandan-rules.js >/dev/null && node scripts/test-guandan-engine.js >/dev/null && node scripts/test-guandan-fuzz.js >/dev/null && node scripts/test-guandan-hint.js >/dev/null && node scripts/journey-guandan-play.js >/dev/null && node scripts/journey-guandan-online.js >/dev/null; then
  pass "掼蛋牌型/级牌抬权/逢人配/炸弹阶 37 项 + 引擎自对弈60局/进贡还贡抗贡/接风/回看重放 75 项 + 模糊压测140局2.8万项不变量 + 提示智能23项(走完优先/剩一对提整对/炸弹垫底/残局对手报单甩大单憋他) + /掼蛋完整旅程(升级延续/进贡/战绩卡) + 真人联机15项(host权威/脱敏快照无泄露·只给张数/客人凭伪状态算合法动作/私牌RLS隔离/非本人回合·牌不在手被拒/收敛一致/进贡明置) 全部通过"
else
  fail "掼蛋回归失败（牌型识别 / 级牌百搭 / 状态机 / 进贡 / 名次结算 / 模糊不变量 / replay一致性 / 命令旅程）"
fi

# ─────────────────────────────────────────
# 1b3e2. 德州扑克引擎(无限注 · 边池 · 全局意识 AI)
# ─────────────────────────────────────────
section "1b3e2. 德州扑克游戏引擎"
...
# ─────────────────────────────────────────
# 1b3f. 游戏卡/牌桌真实渲染布局回归(反"正则测不出显示不全")
# 用 playwright+Chrome 真的渲染到 390px 手机屏, 量横向溢出/裁切; 无 Chrome 时脚本自跳过
# ─────────────────────────────────────────
section "1b3f. 游戏卡/牌桌真实渲染布局"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行可视化回归"
elif node scripts/visual-check-cards.js; then
  pass "大厅卡 host/路人/进行中三视角无横向溢出 + 掼蛋手牌不溢出/不裁切/按钮完整（或无 Chrome 已跳过）"
else
  fail "游戏卡真实渲染布局回归失败（横向溢出 / 牌被裁切 / 按钮被切 —— 看上面哪张卡）"
fi

# ─────────────────────────────────────────
# 1b3h. 牌桌↔聊天室融合·折叠/展开旅程(反"返回即销毁牌局"回退)
# 真渲两桌, 点✕返回后断言牌桌仍在 DOM(未 room.remove)+活牌桌片浮现; 无 Chrome 自跳过
# ─────────────────────────────────────────
section "1b3h. 牌桌折叠/展开(返回不销毁牌局)"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行折叠/展开旅程"
elif node scripts/journey-table-fold.js; then
  pass "斗地主+掼蛋: ✕返回=折叠成右下角活牌桌片(牌局后台续跑, 牌桌不销毁) + 点片子展开回同一局 + 收工才真移除（或无 Chrome 已跳过）"
else
  fail "折叠/展开旅程失败（返回销毁了牌局 / 活牌桌片没浮现 / 展开没回到同一局 —— 看上面哪步红）"
fi

# ─────────────────────────────────────────
# 1b3i. 牌桌↔聊天室融合·边打边聊旅程(坞/弹幕真挂 + 收发真通 + 回声去重)
# 真渲两桌, 派发 onRoomMsg 验弹幕/列表/未读, 提交表单验 send 桥 + 去重; 无 Chrome 自跳过
# ─────────────────────────────────────────
section "1b3i. 牌桌边打边聊(内嵌聊天坞+弹幕)"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行边打边聊旅程"
elif node scripts/journey-table-chat.js; then
  pass "斗地主+掼蛋: 开桌挂出聊天坞+弹幕层 + 房间消息横掠绒面/进坞列表/亮未读 + 从坞发送走现有通道且乐观上屏 + 自发 realtime 回声去重 + 非聊天类不进桌（或无 Chrome 已跳过）"
else
  fail "边打边聊旅程失败（坞/弹幕没挂 / 来消息没进弹幕或列表 / 发送没走 send 桥 / 自发回声重复了 —— 看上面哪步红）"
fi

# ─────────────────────────────────────────
# 1b3j. 牌桌↔聊天室融合·牌局直播旅程(高光 beat 真发出 + 灵魂入戏 + 真人不配台词)
# 真渲两桌, applyMove 同步驱动整局到 over, 收 onBeat 断言战报契约; 无 Chrome 自跳过
# ─────────────────────────────────────────
section "1b3j. 牌桌牌局直播(高光进聊天室+灵魂入戏)"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行牌局直播旅程"
elif node scripts/journey-table-beat.js; then
  pass "斗地主+掼蛋: 一整局高光(定地主/炸弹/报单/头游/终局升级)经 onBeat 真发出 + 每条战报有文案 + 终局/定地主战报到位 + 真人动作绝不配台词/AI 灵魂对手入戏 + 炸弹标高光档（或无 Chrome 已跳过）"
else
  fail "牌局直播旅程失败（beat 没发出 / 缺终局或定地主战报 / 真人被误配台词 / 炸弹没标高光 —— 看上面哪步红）"
fi

# ─────────────────────────────────────────
# 1b3k. 游戏体验升级旅程(连接可见 / 房主离线 / 后台提醒 / 回看重建)
# 真实渲染德州+掼蛋 guest 牌桌；断言断线锁操作、非0座位、恢复在线；源码契约覆盖后台提醒和 replay。
# ─────────────────────────────────────────
section "1b3k. 游戏连接与回看体验"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行游戏体验旅程"
elif node scripts/journey-game-experience.js; then
  pass "德州+掼蛋连接状态可见/断线锁操作/房主离线/非0座位 + 后台提醒 + 三类战报 replay 回看全部通过"
else
  fail "游戏体验旅程失败（连接状态 / 断线锁操作 / 房主离线 / 后台提醒 / 回看重建 —— 看上面哪步红）"
fi

# ─────────────────────────────────────────
# 1b3k2. 三桌视觉一致性 + 融合升级验证
# 断言：字体链不落回 Arial Narrow / 卡牌背面不用斜条纹 / .ddz-felt+.gd-felt+.pk-felt 有绿绒毡 /
# title chip 化 / gd-tag ≥10.5px / 版本号 exp2 同步 / 真机复验斗地主生效。
# ─────────────────────────────────────────
section "1b3k2. 三桌视觉一致性 + 融合升级"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行视觉一致性旅程"
elif node scripts/journey-table-visual.js; then
  pass "三桌字体链修正 / 卡背换新 / 桌面绒毡 / 顶栏 chip / 掼蛋 tag 字号 + 真机复验通过"
else
  fail "三桌视觉一致性/融合升级旅程失败（看上面哪步红）"
fi

# ─────────────────────────────────────────
# 1b3g. 联机牌桌 host 权威同步(隐私隔离 + 收敛 + 权威校验 + 确定性)
# 内存总线跑 host+guest 完整一局, 断言别家手牌不泄漏 / 公共态收敛 / 非法动作被拒
# ─────────────────────────────────────────
section "1b3g. 联机牌桌 host 权威同步"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行联机同步旅程测试"
elif node scripts/journey-table-sync.js >/dev/null; then
  pass "斗地主8局+掼蛋6局全程走 host 权威同步通道: 脱敏快照(别家手牌只留张数,无 seed/log 泄漏) + 全席公共态收敛一致 + 非本人回合/终局后动作被拒 + 出牌只送id由host复原 + log重放确定性一致"
else
  fail "联机同步旅程失败（手牌隐私泄漏 / 公共态不收敛 / 越权动作被放行 / 同步后确定性受损）"
fi
...
# 1b4a. 公开房重复进房成员幂等旅程
# ─────────────────────────────────────────
section "1b4a. 公开房重复进房成员幂等旅程"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行成员幂等旅程测试"
elif node scripts/journey-member-upsert.js; then
  pass "公开房重复进房 upsert + 私密房只读校验 + 旧错误实现反证通过"
else
  fail "公开房成员幂等旅程回归失败"
fi
...
# ─────────────────────────────────────────
# 1b6b. /命令 与 @提及 菜单点选/滑动分离旅程
# ─────────────────────────────────────────
section "1b6b. /命令 · @提及 菜单点选/滑动分离旅程"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行菜单点选/滑动旅程"
elif node scripts/journey-slash-menu-scroll.js; then
  pass "点选/滑动分离 + 防遮挡封顶 + 44px 触控目标 + 滑动不误选反证通过"
else
  fail "/命令 · @提及 菜单点选/滑动分离旅程失败"
fi
...
# ─────────────────────────────────────────
# 1b8b. 恢复上次房间后首页骨架必须触发真实查询
# ─────────────────────────────────────────
section "1b8b. 首页骨架恢复旅程"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行首页骨架恢复旅程"
elif node scripts/journey-lobby-skeleton.js; then
  pass "恢复房间时骨架不误判 / 真实卡保留 soft 快路径 + 旧实现反证通过"
else
  fail "首页骨架恢复旅程失败"
fi
...
# ─────────────────────────────────────────
# 1b17a. 折叠屏展开态主聊天室真实 DOM 坐标旅程
# ─────────────────────────────────────────
section "1b17a. 折叠屏展开态输入框 / 键盘真实坐标"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行折叠屏真实 DOM 坐标旅程"
elif node scripts/journey-kb-foldable-dom.js; then
  pass "折叠屏展开态输入框不越过键盘顶沿、桌面布局不回归、旧宽度门反证通过"
else
  fail "折叠屏展开态真实 DOM 坐标旅程失败"
fi
...
# 1b18. 灵魂游戏交互生命周期
# ─────────────────────────────────────────
section "1b18. 灵魂游戏交互生命周期"

if ! command -v node >/dev/null 2>&1; then
  fail "node 未安装，无法运行灵魂游戏生命周期旅程"
elif node scripts/journey-soul-interact-lifecycle.js; then
  pass "灵魂游戏过期 / 记忆窗口 / 离线 / 空房 + 旧实现反证通过"
else
  fail "灵魂游戏交互生命周期旅程失败"
fi
